...
- `THREAT_MODEL.md` gained an "Engineering controls" table: maps each protection (no external calls, prompt-injection boundary, mid-stream harmful-output check, SQL whitelists, write gate, atomic writes, `OLLAMA_HOST` validation, non-root container) to the file that enforces it, so reviewers can verify a change has not weakened a control. Added after a periodic security re-audit (2026-06-30) confirmed all controls present and no new findings
