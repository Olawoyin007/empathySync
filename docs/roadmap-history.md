...
Completed-phase detail moved out of `ROADMAP.md` to keep the roadmap focused on
planned work. This file is the verbatim record of every completed phase
(implementation notes, sub-phase checklists, point-in-time accuracy numbers),
plus the original text of retired plans. For what shipped in each release, see
`CHANGELOG.md`.
...
Original text of phases that were retired or folded into other phases
(2026-07-03). Kept verbatim for the record.

## Phase 10 (folded into Phases 21 and 22)

Semantic intent detection (10.1) and the model-agnostic safety layer (10.3)
are Phase 21 by other names; conversation flow analysis (10.2) belongs to the
Phase 22 pattern tracking. Original text:
...
---
## Phase 18 (retired - contradicted the local-first manifesto)

WhatsApp and Slack route conversations through third-party servers, which the
deferral note below already conceded. Retired outright rather than kept as a
detailed plan; a short Signal-only note remains in ROADMAP.md. Original text:
...
## Phase 20 (original installer/code-signing plan, replaced by a package-manager plan)

The original plan required platform installers (briefcase/PyInstaller) plus
Windows EV and Apple code-signing certificates - recurring cost and heavy
maintenance for a solo-maintained OSS project. Replaced in ROADMAP.md with a
package-manager distribution plan. Original text:
...
## Phase 21: Safety Classifier Upgrade ✅ COMPLETE (issue #125; 21.1-21.4 shipped in v1.13.0)
